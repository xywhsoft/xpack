/*
 * xPack Ver7 - Rebuild Operations (16)
 */
...
TEST(rebuild_multiple_cycles) {
    ...
    xpkObject xpk = xpkOpen(filename, 0, 1);
    ASSERT_NOT_NULL(xpk);
...
void register_16_rebuild_operations_tests(void) {
    TEST_REGISTER(rebuild_empty_package, CAT_CORE, "Rebuild empty package");
    TEST_REGISTER(rebuild_single_file, CAT_CORE, "Rebuild single file");
    TEST_REGISTER(rebuild_multiple_files, CAT_CORE, "Rebuild multiple files");
    TEST_REGISTER(rebuild_after_remove, CAT_CORE, "Rebuild after remove");
    TEST_REGISTER(rebuild_after_update, CAT_CORE, "Rebuild after update");
    TEST_REGISTER(rebuild_multiple_cycles, CAT_CORE, "Rebuild multiple cycles");
    TEST_REGISTER(rebuild_large_files, CAT_CORE, "Rebuild large files");
    TEST_REGISTER(rebuild_path_mode, CAT_CORE, "Rebuild path mode");
    TEST_REGISTER(rebuild_index_mode, CAT_CORE, "Rebuild index mode");
    TEST_REGISTER(rebuild_empty_files, CAT_CORE, "Rebuild empty files");
    TEST_REGISTER(rebuild_mixed_levels, CAT_CORE, "Rebuild mixed levels");
}
