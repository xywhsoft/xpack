...
typedef struct {
    ...
    // === LDB 信息 (12 bytes) ===
    uint32_t    ldbRawSize;         // 0x28: LDB 原始大小
    uint32_t    ldbCompSize;        // 0x2C: LDB 压缩后大小
    uint32_t    ldbHash;            // 0x30: LDB 哈希值

    // === 时间戳 (16 bytes) ===
    xtime       createTime;         // 0x34: 创建时间
    xtime       changeTime;         // 0x3C: 修改时间
} xpkHead;
#pragma pack(pop)
