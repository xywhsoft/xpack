...
#define AVL_MAX_HEIGHT  42 /* AVL 树最大高度
...
int avl_destroy(AVL_TREE root, AVL_FREE free_rtn)
{
    ...
    /* walk left side */
    if (!(NULL == root->left)) {
        if (avl_destroy (root->left, free_rtn) == AVL_RET_ERROR)
        return (AVL_RET_ERROR);
    }

    /* walk right side */
    if (!(NULL == root->right)) {
        if (avl_destroy (root->right, free_rtn) == AVL_RET_ERROR)
            return (AVL_RET_ERROR);
    }

    /* call free routine */
    if (free_rtn != NULL)
        if (free_rtn (root) == AVL_RET_ERROR)
            return (AVL_RET_ERROR);
                  ...
                  AVL_CALLBACK inRtn, void *inArg, AVL_CALLBACK postRtn, void *postArg)
{
    ...
    if (preRtn != NULL)
        if (preRtn (root, preArg) == AVL_RET_ERROR)
            return (AVL_RET_ERROR);

    /* walk left side */
    if (!(NULL == root->left)) {
        if (avl_tree_walk (root->left, preRtn, preArg, inRtn, inArg,
                 postRtn, postArg) == AVL_RET_ERROR)
        return (AVL_RET_ERROR);
    }

    /* call in-order routine */
    if (inRtn != NULL)
        if (inRtn (root, inArg) == AVL_RET_ERROR)
            return (AVL_RET_ERROR);
    ...
    if (postRtn != NULL)
        if (postRtn (root, postArg) == AVL_RET_ERROR)
            return (AVL_RET_ERROR);
                    ...
                    void **ppKeyHolder, AVL_COMPARE compare)
{
    ...
    while (TRUE) {
        ...
        if  (0 == delta) {
            /* we inform the caller of the key holder node and return AVL_RET_ERROR */
