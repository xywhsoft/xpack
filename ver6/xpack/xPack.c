...
#include "../mmu/mmu.h"
#include "../xxhash32/xxhash.h"



// 区分只读版本和读写版本
#ifdef XPK_BUILD_READONLY
	#include "../lz4dec/lz4.h"
...
#define xProc_OnError_Router(err)	xCore.LastErrorID = err; xCore.LastError = xPack_Error_Text[err - 1]; return 0;
...
XXAPI int xPack_Compress_Router(xPackObject xpk, xPack_CompInfo* pInfo)
{
	...
	if ( lv == XPK_COMP_FAST ) {
		...
		ptr pOut = xCore.malloc(iDstSize);
		...
		// 高压缩比（LZMA）
		ptr pOut = xCore.malloc(pInfo->SrcSize);
		if ( pOut == 0 ) { xPack_OnError_Router(xpk, 3); xProc_OnError_Router(3) }
...
XXAPI int xPack_DeCompress_Router(xPackObject xpk, xPack_CompInfo* pInfo)
{
	...
	if ( lv == XPK_COMP_FAST ) {
		// 快速压缩（LZ4）
		astr pOut = xCore.malloc(pInfo->DstSize + 4);
		if ( pOut == 0 ) { xPack_OnError_Router(xpk, 3); xProc_OnError_Router(3) }
...
XXAPI int xPack_Save(xPackObject xpk, int bReBuild)
{
	...
	if ( xpk->PackHead.FileCount ) {
		// 计算 LDB 哈希值
		uint LDB_Size = xpk->LDB->ItemLenght * xpk->PackHead.FileCount;
		xpk->PackHead.LDB_Hash = XXH32(xpk->LDB->Memory, LDB_Size, 0);
		// 压缩文件列表
		...
		// 写入文件列表
		bRet = xFile_Seek(xpk->FileObject, xpk->FileOffset + xpk->PackHead.LDB_Addr, FILE_BEGIN);
		if ( bRet == false ) { if ( CompInfo.FreeData ) { xCore.free(CompInfo.DstAddr); } xPack_OnError_Router(xpk, 10); xProc_OnError_Router(10); }
		ulong iRet = xFile_Put(xpk->FileObject, CompInfo.DstAddr, CompInfo.DstSize);
		if ( iRet != CompInfo.DstSize ) { if ( CompInfo.FreeData ) { xCore.free(CompInfo.DstAddr); } xPack_OnError_Router(xpk, 9); xProc_OnError_Router(9); }
		if ( CompInfo.FreeData ) { xCore.free(CompInfo.DstAddr); }
	...
	// 写入文件头数据
	bool bRet = xFile_Seek(xpk->FileObject, xpk->FileOffset, FILE_BEGIN);
	if ( bRet == false ) { xPack_OnError_Router(xpk, 10); xProc_OnError_Router(10); }
	ulong iRet = xFile_Put(xpk->FileObject, &xpk->PackHead, sizeof(xPack_FileHead));
...
XXAPI void xPack_Close(xPackObject xpk)
{
	...
	// 清理数据
	if ( xpk->FileObject ) { xFile_Close(xpk->FileObject); }
	if ( xpk->LDB ) { SMMU_Destroy(xpk->LDB); }
	xCore.free(xpk);
}
...
{
	// 文件不存在则创建
	xPackObject xpk = xCore.malloc(sizeof(xPackStruct));
	if ( xpk == 0 ) { xProc_OnError_Router(3) }
	memset(xpk, 0, sizeof(xPackStruct));
	if ( (bReadOnly == 0) && (xFile_ExistsA(sFile) == 0) ) {
		xpk->PackHead.FileHead = XPK_VERSION;
		...
		ulong iRet = xFile_PutAllA(sFile, &xpk->PackHead, sizeof(xPack_FileHead));
		if ( iRet != sizeof(xPack_FileHead) ) { xPack_OnError_Router(xpk, 9); xProc_OnError_Router(9); }
	}
	// 打开文件
	xpk->FileOffset = iOffset;
	xpk->FileObject = xFile_OpenA(sFile, bReadOnly, CHARSET_BINARY);
	if ( xpk->FileObject == 0 ) {
		xCore.free(xpk);
		xProc_OnError_Router(1)
	}
	// 读取信息段，验证文件头和版本
	bool bRet = xFile_Seek(xpk->FileObject, iOffset, FILE_BEGIN);
	if ( bRet == false ) { xFile_Close(xpk->FileObject); xCore.free(xpk); xProc_OnError_Router(10); }
	int iRet = xFile_Get(xpk->FileObject, &xpk->PackHead, sizeof(xPack_FileHead));
	if ( iRet != sizeof(xPack_FileHead) ) { xFile_Close(xpk->FileObject); xCore.free(xpk); xProc_OnError_Router(8); }
	if ( xpk->PackHead.FileHead != XPK_VERSION ) {
		xFile_Close(xpk->FileObject);
		xCore.free(xpk);
		xProc_OnError_Router(2)
	}
	// 读取文件列表(LDB段)
	xpk->LDB = SMMU_Create(sizeof(xPack_FileInfo) + xpk->PackHead.InfoSize, 32);
	if ( xpk->LDB == 0 ) {
		xFile_Close(xpk->FileObject);
		xCore.free(xpk);
	...
	// 扩展 LDB 段内存
	if ( SMMU_Malloc(xpk->LDB, xpk->PackHead.FileCount) == 0 ) {
		xPack_Close(xpk);
		xProc_OnError_Router(3)
	}
	...
	if ( xpk->PackHead.FileCount ) {
		...
		if ( xpk->PackHead.PackFlag & XPK_LDBCOMP ) {
			// 解压文件列表(LDB段)
			ptr LDB_Data = xCore.malloc(xpk->PackHead.LDB_Size);
			if ( LDB_Data == 0 ) { xPack_Close(xpk); xProc_OnError_Router(3) }
			bRet = xFile_Seek(xpk->FileObject, iOffset + xpk->PackHead.LDB_Addr, FILE_BEGIN);
			if ( bRet == false ) { xPack_Close(xpk); xProc_OnError_Router(10); }
			...
			if ( CompInfo.FreeData ) { xCore.free(LDB_Data); }
			memcpy(xpk->LDB->Memory, CompInfo.DstAddr, CompInfo.DstSize);
			xCore.free(CompInfo.DstAddr);
		} else {
			// 无需解压，直接读取 LDB 段
			bRet = xFile_Seek(xpk->FileObject, iOffset + xpk->PackHead.LDB_Addr, FILE_BEGIN);
			if ( bRet == false ) { xPack_Close(xpk); xProc_OnError_Router(10); }
...
{
	if ( xpk && xpk->LDB ) {
		return SMMU_GetPtr(xpk->LDB, iPos);
	}
	return NULL;
}
...
XXAPI uint xPack_Core_AppendData(xPackObject xpk, ptr pIn, uint iSize, int iCompLevel)
{
	if ( xpk && xpk->LDB ) {
		// 创建文件信息
		uint iPos = SMMU_Append(xpk->LDB, 1);
		xPack_FileInfo* pInfo = SMMU_GetPtr(xpk->LDB, iPos);
		...
		if ( iSize > 0 ) {
			...
			// 写入文件数据
			bRet = xFile_Seek(xpk->FileObject, xpk->FileOffset + xpk->PackHead.LDB_Addr, FILE_BEGIN);
			if ( bRet == false ) { SMMU_Delete(xpk->LDB, iPos, 1); if ( CompInfo.FreeData ) { xCore.free(pIn); } xPack_OnError_Router(xpk, 10); xProc_OnError_Router(10); }
			int iRet = xFile_Put(xpk->FileObject, pIn, iSize);
			if ( iRet != iSize ) { SMMU_Delete(xpk->LDB, iPos, 1); if ( CompInfo.FreeData ) { xCore.free(pIn); } xPack_OnError_Router(xpk, 9); xProc_OnError_Router(9); }
			if ( CompInfo.FreeData ) { xCore.free(pIn); }
		}
		// 更新文件信息
...
// 添加文件（核心模式）
XXAPI uint xPack_Core_AppendFile(xPackObject xpk, astr sFile, int iCompLevel)
{
	// 读取文件到内存
	...
	ptr pData = NULL;
	if ( iSize ) {
		pData = xCore.malloc(iSize);
		if ( pData == 0 ) { xFile_Close(objFile); xPack_OnError_Router(xpk, 3); xProc_OnError_Router(3); }
		ulong iRet = xFile_Get(objFile, pData, iSize);
		if ( iRet != iSize ) { xFile_Close(objFile); if ( pData ) { xCore.free(pData); } xPack_OnError_Router(xpk, 8); xProc_OnError_Router(8); }
	}
	xFile_Close(objFile);
	// 添加文件数据
	uint iRet = xPack_Core_AppendData(xpk, pData, iSize, iCompLevel);
	if ( pData ) { xCore.free(pData); }
	return iRet;
}
...
// 修改文件（核心模式）
XXAPI xPack_FileInfo* xPack_Core_ChangeFile(xPackObject xpk, uint iPos, astr sFile, int iCompLevel)
{
	// 读取文件到内存
	xFileObject objFile = xFile_OpenA(sFile, TRUE, CHARSET_BINARY);
	if ( objFile == 0 ) { xPack_OnError_Router(xpk, 1); xProc_OnError_Router(1); }
...
// 解包数据（核心模式）（pData需使用 xCore.free 释放）
XXAPI xPack_FileInfo* xPack_Core_UnpackData(xPackObject xpk, uint iPos, ptr* pData)
{
	if ( xpk && xpk->LDB ) {
		...
		xPack_FileInfo* pInfo = SMMU_GetPtr(xpk->LDB, iPos);
		if ( pInfo == NULL ) { xPack_OnError_Router(xpk, 6); xProc_OnError_Router(6); }
		if ( pInfo->FileSize == 0 ) { *pData = xCore.nullstring; return pInfo; }
		// 读取文件内容
		uint iSize = pInfo->DataSize;
		astr pOut = xCore.malloc(iSize + 4);
		if ( pOut == 0 ) { xPack_OnError_Router(xpk, 3); xProc_OnError_Router(3); }
		bool bRet = xFile_Seek(xpk->FileObject, xpk->FileOffset + pInfo->DataAddr, FILE_BEGIN);
		if ( bRet == false ) { xCore.free(pOut); xPack_OnError_Router(xpk, 10); xProc_OnError_Router(10); }
		ulong iRet = xFile_Get(xpk->FileObject, pOut, iSize);
		if ( iRet != iSize ) { xCore.free(pOut); xPack_OnError_Router(xpk, 8); xProc_OnError_Router(8); }
		// 解压数据
		if ( pInfo->FileFlag & XPK_COMP_CUSTOM ) {
			...
			xPack_DeCompress_Router(xpk, &CompInfo);
			if ( CompInfo.FreeData ) { xCore.free(pOut); }
...
// 解包文件（核心模式）
XXAPI xPack_FileInfo* xPack_Core_UnpackFile(xPackObject xpk, uint iPos, astr sFile)
{
	ptr pOut = NULL;
	...
	if ( pInfo ) {
		if ( pInfo->FileSize > 0 ) {
			...
			if ( iRet != pInfo->FileSize ) { xCore.free(pOut); xPack_OnError_Router(xpk, 9); xProc_OnError_Router(9); }
		} else {
			// 创建空文件
...
XXAPI int xPack_Core_DeleteFile(xPackObject xpk, uint iPos)
{
	if ( xpk && xpk->LDB ) {
		// 删除文件信息
		return SMMU_Delete(xpk->LDB, iPos, 1);
	}
	return 0;
}
...
XXAPI uint xPack_IndexToPos(xPackObject xpk, int iIndex)
{
	if ( xpk && xpk->LDB ) {
		...
		for ( int i = 0; i < iCount; i++ ) {
			xPack_FileInfo_Index* pInfo = (ptr)(&xpk->LDB->Memory[i * xpk->LDB->ItemLenght]);
...
// 添加文件（无序索引方式）
XXAPI xPack_FileInfo_Index* xPack_Index_AppendFile(xPackObject xpk, int iIndex, astr sFile, int iCompLevel)
{
	...
	if ( iPos ) {
		xPack_FileInfo_Index* pInfo = SMMU_GetPtr(xpk->LDB, iPos);
...
// 修改文件（无序索引方式）
XXAPI xPack_FileInfo_Index* xPack_Index_ChangeFile(xPackObject xpk, int iIndex, astr sFile, int iCompLevel)
{
	if ( xpk == 0 ) { return NULL; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Index ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
...
// 解包文件（无序索引方式）
XXAPI xPack_FileInfo_Index* xPack_Index_UnpackFile(xPackObject xpk, int iIndex, astr sFile)
{
	if ( xpk == 0 ) { return NULL; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Index ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_IndexToPos(xpk, iIndex);
...
// 通过 Linux文件路径 获取 POS
XXAPI uint xPack_LinuxToPos(xPackObject xpk, astr sPath)
{
	if ( xpk && xpk->LDB ) {
		uint iCount = xpk->LDB->Count;
		uint iHash = XXH32(sPath, strlen(sPath), 0);
		for ( int i = 0; i < iCount; i++ ) {
			xPack_FileInfo_Linux* pInfo = (ptr)(&xpk->LDB->Memory[i * xpk->LDB->ItemLenght]);
...
// 添加文件（Linux方式）
XXAPI xPack_FileInfo_Linux* xPack_Linux_AppendFile(xPackObject xpk, astr sPath, astr sFile, int iCompLevel)
{
	if ( xpk == 0 ) { return 0; }
	...
	uint iPos = xPack_Core_AppendFile(xpk, sFile, iCompLevel);
	if ( iPos ) {
		xPack_FileInfo_Linux* pInfo = SMMU_GetPtr(xpk->LDB, iPos);
		if ( pInfo == 0 ) { return NULL; }
...
// 修改文件（Linux方式）
XXAPI xPack_FileInfo_Linux* xPack_Linux_ChangeFile(xPackObject xpk, astr sPath, astr sFile, int iCompLevel)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Linux ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
...
// 解包文件（Linux方式）
XXAPI xPack_FileInfo_Linux* xPack_Linux_UnpackFile(xPackObject xpk, astr sPath, astr sFile)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Linux ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_LinuxToPos(xpk, sPath);
...
// 解包数据（Linux方式）
XXAPI xPack_FileInfo_Linux* xPack_Linux_UnpackData(xPackObject xpk, astr sPath, ptr* pData)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Linux ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_LinuxToPos(xpk, sPath);
...
// 删除文件（Linux方式）
XXAPI int xPack_Linux_DeleteFile(xPackObject xpk, astr sPath)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Linux ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_LinuxToPos(xpk, sPath);
...
XXAPI uint xPack_Win32ToPos(xPackObject xpk, astr sPath)
{
	if ( xpk && xpk->LDB ) {
		uint iCount = xpk->LDB->Count;
		sPath = xCore_LCaseA(sPath, 0, FALSE);
		uint iHash = XXH32(sPath, strlen(sPath), 0);
		for ( int i = 0; i < iCount; i++ ) {
			xPack_FileInfo_Linux* pInfo = (ptr)(&xpk->LDB->Memory[i * xpk->LDB->ItemLenght]);
			if ( pInfo->PathHash == iHash ) {
...
// 添加文件（Win32方式）
XXAPI xPack_FileInfo_Win32* xPack_Win32_AppendFile(xPackObject xpk, astr sPath, astr sFile, int iCompLevel)
{
	if ( xpk == 0 ) { return 0; }
	...
	uint iPos = xPack_Core_AppendFile(xpk, sFile, iCompLevel);
	if ( iPos ) {
		xPack_FileInfo_Win32* pInfo = SMMU_GetPtr(xpk->LDB, iPos);
		if ( pInfo == 0 ) { return NULL; }
		memset(pInfo->FilePath, 0, XPK_FILEPATHMAX);
		strcpy(pInfo->FilePath, sPath);
		astr sLowPath = xCore_LCaseA(sPath, iSize, FALSE);
...
// 修改文件（Win32方式）
XXAPI xPack_FileInfo_Win32* xPack_Win32_ChangeFile(xPackObject xpk, astr sPath, astr sFile, int iCompLevel)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Win32 ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
...
// 解包文件（Win32方式）
XXAPI xPack_FileInfo_Win32* xPack_Win32_UnpackFile(xPackObject xpk, astr sPath, astr sFile)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Win32 ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_Win32ToPos(xpk, sPath);
...
// 解包数据（Win32方式）
XXAPI xPack_FileInfo_Win32* xPack_Win32_UnpackData(xPackObject xpk, astr sPath, ptr* pData)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Win32 ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_Win32ToPos(xpk, sPath);
...
// 删除文件（Win32方式）
XXAPI int xPack_Win32_DeleteFile(xPackObject xpk, astr sPath)
{
	if ( xpk == 0 ) { return 0; }
	if ( (xpk->PackHead.PackFlag & XPK_CLASS_MASK) != XPK_CLASS_Win32 ) { xPack_OnError_Router(xpk, 11); xProc_OnError_Router(11); }
	uint iPos = xPack_Win32ToPos(xpk, sPath);
