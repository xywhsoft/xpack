...
- [ ] 编写手工回归测试清单，覆盖 GUI 和 Explorer 场景。
- [ ] 手工验证新建 `core` 包并打开。
- [ ] 手工验证新建 `index` 包并打开。
- [ ] 手工验证新建 `linux` 包并打开。
- [ ] 手工验证新建 `win32` 包并打开。
...
- [ ] 确认 DLL 被 Explorer 锁定时的重新构建提示或处理文档。
- [ ] 增加 shell 扩展注册失败时的诊断输出。
- [ ] 增加 shell 扩展卸载后的残留注册表检查说明。
...
- [x] 支持列排序和列宽持久化。
- [ ] 增加列显示/隐藏配置。
- [ ] 增加 `View -> Columns...` 或等价列配置入口。
- [ ] 增加 `View -> Show Grid Lines` 等列表显示偏好。
- [ ] 增加 `View -> Full Row Select` 等列表显示偏好。
- [ ] 增加复制完整行信息时的列顺序与可见列策略。
- [ ] 增加更接近 7-Zip 的归档属性摘要，例如压缩率、物理大小、头部/表大小、空洞占比。
- [ ] 增加工具栏按钮的 enable/disable 状态细化。
- [ ] 增加菜单项分组和名称最终审查，避免用户混淆 `Edit` 与 `Edit Text...`。
...
- [ ] 梳理 `xpack.h` 当前公开 API，确认 GUI 是否遗漏新 API。
- [ ] 如果 xPack 后续新增 API，先更新本节再接入 GUI。
- [ ] 增强 `volume` 模式的 GUI 提示，明确分卷输出和限制。
- [ ] 增强 `solid` 模式的 GUI 提示，明确部分操作限制或成本。
- [ ] 增加 pack type 转换的风险提示，避免用户误把归档类型切换导致数据语义变化。
- [ ] 增加 `index` 包批量 fileIndex 调整或重新编号能力评估。
- [ ] 增加 `linux` / `win32` 包时间戳编辑能力评估。
...
- [ ] 统一错误弹窗文本，区分用户取消、格式错误、IO 错误、API 状态错误。
- [ ] 对 `invalid xpk package format` 增加更明确上下文，例如当前打开的是归档还是临时解压文件。
- [ ] 对临时目录创建失败、外部程序启动失败提供可复制诊断信息。
- [ ] 对外部编辑后写回失败提供保留临时文件路径的提示。
- [ ] 对只读文件、只读归档、权限不足提供更明确提示。
- [ ] 审查所有 `return FALSE` 路径，补齐必要错误提示。
- [ ] 审查所有临时文件/目录生命周期，避免长期堆积或过早删除。
...
- [x] 增加快捷键帮助。
- [x] 增加命令行帮助。
- [ ] 增加状态栏分栏显示，避免单行文本过长时难以阅读。
- [ ] 增加大归档加载时的忙碌提示或后台加载评估。
- [ ] 增加列表空状态提示，例如无归档、目录为空、过滤无结果。
- [ ] 增加过滤框 placeholder 或旁边说明，提示支持通配符和多关键词。
- [ ] 增加进度对话框取消后的结果说明。
- [ ] 增加常用按钮 tooltip。
- [ ] 增加高 DPI 和不同缩放比例下的布局检查。
- [ ] 增加窗口最小尺寸限制。
- [ ] 增加深色模式评估，暂不作为第一版必需项。
...
- [x] 新增本 spec 用于后续进度跟踪。
- [ ] 将 README 拆分为用户使用说明和开发说明，降低单文件长度。
- [ ] 增加 Explorer 右键菜单安装、卸载、重启 Explorer 的排错说明。
- [ ] 增加常见错误 FAQ。
- [ ] 增加手工回归测试说明。
- [ ] 增加发布前检查清单。
...
- [ ] 清理或忽略 `tools/xpkgui/obj` 下临时构建产物。
- [ ] 清理或忽略 `release/x64` 下 smoke 临时输出。
- [ ] 确认 `.gitignore` 覆盖 xpkgui 构建中间文件。
