...
echo "========================================"
echo "Building xpkgui (Linux, dynamic)"
echo "========================================"
echo
...
LIB_LDFLAGS="-shared -Wl,-soname,libxpack.so"
...
$CC -o "$LIB_OUTPUT" $CFLAGS $LIB_LDFLAGS "${SOURCES[@]}" -lm -lpthread
...
echo
echo "Building xpkgui..."
$CC -o "$OUTPUT" -I../../lib -I../../src -I"$OUTPUT_DIR" \
	-DXPK_BUILD_DLL -DZSTD_NO_INTRINSICS -DZ7_ST -O2 -Wall \
	$(pkg-config --cflags gtk+-3.0 2>/dev/null || echo '') \
	xpkgui.c -L"$OUTPUT_DIR" -lxpack -Wl,-rpath,"$OUTPUT_DIR" \
	$(pkg-config --libs gtk+-3.0 2>/dev/null || echo '')
...
if [ $? -eq 0 ]; then
	...
	echo "Note: xpkgui requires libxpack.so to run."
	echo "========================================"
	chmod +x "$OUTPUT"
else
	echo
